...
export class EdcClientService implements OnDestroy {
  ...
  public readonly EDC_API_TOKEN_COOKIE_NAME = 'edc-api-token';
  ...
  public createEdcConnectorClient(config: EdcConfig): EdcConnectorClient {
    ...
    const apiCookie = this.getCookie();
    if (apiCookie) {
      connector.apiToken(apiCookie);
    }
  ...
  public getCookie(): string | null {
    const target = encodeURIComponent(this.EDC_API_TOKEN_COOKIE_NAME) + '=';
